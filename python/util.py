...
# Module version. Bump especially when changing existing functionality.
__version__ = '0.2.0'
...
def itersOfN(iterable, n):
    '''
    Yield iterators that will iterate of the next n items in `iterable` until
    `iterable` has no more items.  The last iterator yielded might yield less
    than n items if the length of `iterable` is not divisible by n.

    An example:

        import util
        for i, it in enumerate(util.itersOfN(range(5), 2)):
            print 'start sub-iterator #{}'.format(i)
            for item in it:
                print item

        start sub-iterator #0
        0
        1
        start sub-iterator #1
        2
        3
        start sub-iterator #2
        4

    iterable: some iterable collection of items
    n: the number of items that each yielded subiterator will yield
    '''
    assert n > 0
    for k, subiter in itertools.groupby(enumerate(iterable), lambda e: e[0] // n):
        def gen_n():
            for i, item in subiter:
                yield item
        yield gen_n()
